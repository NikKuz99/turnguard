...
// RequestCaptchaCallback is called when VK requires captcha.
// On Android: this was a JNI callback to CaptchaActivity.
// On desktop: set to open browser or use auto-solver.
var RequestCaptchaCallback func(redirectURI string) string = func(redirectURI string) string {
	util.TurnLog("[Captcha] No captcha callback set! Cannot solve: %s", redirectURI[:80])
	return ""
}
...
func fetchVkCreds(ctx context.Context, link string) (string, string, string, error) {
	...
	for _, creds := range vkCredentialsList {
		...
		if strings.Contains(err.Error(), "error_code:29") || strings.Contains(err.Error(), "Rate limit") {
			util.TurnLog("[VK Auth] Rate limit detected, trying next credentials...")
		}
	}
...
func getTokenChain(ctx context.Context, link string, creds VKCredentials, client tlsclient.HttpClient, profile Profile) (string, string, string, error) {
	...
	doRequest := func(data string, url string) (resp map[string]interface{}, err error) {
		...
		defer func() {
			if closeErr := httpResp.Body.Close(); closeErr != nil {
				util.TurnLog("close response body: %s", closeErr)
			}
		}()
	...
	if err != nil {
		util.TurnLog("[VK Auth] Token 1 request failed: %v", err)
		return "", "", "", err
	}
	if errMsg, ok := resp["error"].(map[string]interface{}); ok {
		util.TurnLog("[VK Auth] Token 1 VK API error: %v", errMsg)
	...
	util.TurnLog("[VK Auth] Token 1 (anonym_token) received")
	...
	if err != nil {
		util.TurnLog("[VK Auth] getCallPreview request failed: %v", err)
	} else {
		util.TurnLog("[VK Auth] getCallPreview completed (optional)")
	}
	...
	for attempt := 0; ; attempt++ {
		...
		if errObj, hasErr := resp["error"].(map[string]interface{}); hasErr {
			...
			if captchaErr != nil && captchaErr.IsCaptchaError() {
				...
				if !hasSolveMode {
					util.TurnLog("[STREAM %d] [Captcha] No more solve modes available (attempt %d)", streamID, attempt+1)
					...
					//	util.TurnLog("[STREAM %d] [FATAL] 0 connected streams and captcha solve modes exhausted.", streamID)
				...
				switch solveMode {
				case captchaSolveModeAuto:
					util.TurnLog("[Captcha] Attempt 1. Try auto solving...")
					if captchaErr.SessionToken != "" && captchaErr.RedirectURI != "" {
						...
						if solveErr != nil {
							util.TurnLog("[STREAM %d] [Captcha] Auto captcha failed: %v", streamID, solveErr)
						}
					} else {
						solveErr = fmt.Errorf("missing fields for auto solve")
					}
				case captchaSolveModeSliderPOC:
					util.TurnLog("[Captcha] Attempt 2. Try slider solving...")
					if captchaErr.SessionToken != "" && captchaErr.RedirectURI != "" {
						successToken, solveErr = solveVkCaptcha(ctx, captchaErr, streamID, client, profile, true)
						if solveErr != nil {
							util.TurnLog("[STREAM %d] [Captcha] Auto captcha slider POC failed: %v", streamID, solveErr)
						}
					} else {
						solveErr = fmt.Errorf("missing fields for slider POC auto solve")
					}
				case captchaSolveModeManual:
					util.TurnLog("[STREAM %d] [Captcha] Triggering manual captcha fallback...", streamID)
				...
				if solveErr != nil {
					util.TurnLog("[STREAM %d] [Captcha] %s failed (attempt %d): %v", streamID, captchaSolveModeLabel(solveMode), attempt+1, solveErr)

					nextSolveMode, hasNextSolveMode := captchaSolveModeForAttempt(attempt+1, manualCaptcha, autoCaptchaSliderPOC)
					if hasNextSolveMode {
						util.TurnLog("[STREAM %d] [Captcha] Falling back to %s...", streamID, captchaSolveModeLabel(nextSolveMode))
						continue
					}
					...
					//if connectedStreams.Load() == 0 {
					//	util.TurnLog("[STREAM %d] [FATAL] 0 connected streams and manual captcha failed/timed out.", streamID)
					//	return "", "", "", fmt.Errorf("FATAL_CAPTCHA_FAILED_NO_STREAMS")
					//}
	...
	util.TurnLog("[VK Auth] Token 2 (messages token) received")
	...
	util.TurnLog("[VK Auth] Token 3 (session_key) received")
	...
	util.TurnLog("[VK Auth] TURN credentials received")
	...
	if err == nil {
		if ip := net.ParseIP(host); ip == nil {
			resolvedIP, err := vkHosts.Resolve(ctx, host)
			if err != nil {
				util.TurnLog("[TURN DNS] Warning: failed to resolve TURN server %s: %v", host, err)
			} else {
				address = net.JoinHostPort(resolvedIP, port)
				util.TurnLog("[TURN DNS] Resolved TURN server %s -> %s", host, resolvedIP)
			}
		}
	}
