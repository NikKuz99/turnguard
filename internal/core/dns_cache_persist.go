 ...
 * Format: JSON file at path provided by SetCachePath.
 *
...
const (
	...
	saveDebounce       = 5 * time.Second   // debounce async saves (unused, kept for symmetry)
	saveInterval       = 60 * time.Second   // periodic save while app is running
...
// SetCachePath loads cache from disk synchronously and starts background saver.
// Called from main.go at startup.
func (p *PersistentCache) SetCachePath(path string) {
	util.TurnLog("[DNSCache] SetCachePath called: %s", path)
	...
	if err := p.loadFromDisk(); err != nil {
		util.TurnLog("[DNSCache] load error: %v (starting with empty cache)", err)
	} else {
		p.mu.Lock()
		p.loaded = true
		p.mu.Unlock()
...
func (p *PersistentCache) loadFromDisk() error {
	...
	if cf.Version != 1 {
		util.TurnLog("[DNSCache] unknown version %d, ignoring", cf.Version)
		return nil
	}
	...
	util.TurnLog("[DNSCache] loaded %d domains from disk", len(cf.Domains))
	return nil
}
...
// SaveNow forces a synchronous save.
...
func (p *PersistentCache) saveToDisk() error {
	...
	// Snapshot metrics for trimByScore
...
func (p *PersistentCache) startSaverLocked() {
	...
	go func(stopCh <-chan struct{}) {
		...
		util.TurnLog("[DNSCache] saver goroutine started (interval=%v)", saveInterval)
...
// ─────────────────────────────────────────────────────────────────────────────
// Eviction: hard-failed IPs are removed when FailCount hits limit.
// ─────────────────────────────────────────────────────────────────────────────
