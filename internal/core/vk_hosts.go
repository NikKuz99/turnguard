...
type VkHosts struct {
	...
	pendingFailures map[string][]string    // домен → буфер неудач (TTL списывается после успеха)
	pendingStarted  map[string]time.Time   // домен → когда начали буфер (для авто-дискарда)
}
