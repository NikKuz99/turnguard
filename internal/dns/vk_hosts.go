...
func (vh *VkHosts) Resolve(ctx context.Context, domain string) (string, error) {
	...
	if vh.dnsOK {
		...
		if err == nil && len(ips) > 0 {
			...
			if addedCount > 0 {
				util.TurnLog("[VKHosts] DNS added %d new IPs for %s (total: %d): %v",
					addedCount, domain, len(vh.dynamic[domain]), vh.dynamic[domain])
			}
			if wasDNSFailed {
				util.TurnLog("[VKHosts] DNS recovered for %s — switching back from baseline to DNS mode", domain)
			}
			vh.mu.Unlock()
		...
		vh.dnsOK = false
		vh.mu.Unlock()
		util.TurnLog("[VKHosts] DNS failed for %s, falling back to baseline+dynamic", domain)
	}
...
func (vh *VkHosts) selectBestIP(domain string) (string, error) {
	...
	if bestIP == "" {
		...
		if baseline := vkHostsBaseline[domain]; len(baseline) > 0 {
			bestIP = baseline[0]
			util.TurnLog("[VKHosts] All IPs blacklisted for %s, using baseline: %s", domain, bestIP)
		} else {
...
func (vh *VkHosts) UpdateDynamic(domain string, ips []string) {
	...
	if len(ips) > 0 {
		...
		vh.dnsOK = true
		vh.lastDNS = time.Now()
		util.TurnLog("[VKHosts] Updated dynamic IPs for %s: %v", domain, ips)
	}
}
...
func (vh *VkHosts) MarkDNSWorking() {
	...
	if !vh.dnsOK {
		vh.dnsOK = true
		vh.lastDNS = time.Now()
		util.TurnLog("[VKHosts] DNS is working again")
	}
}
...
func (vh *VkHosts) probeIP(domain, ip string) {
	...
	if err != nil {
		...
		util.TurnLog("[VKHosts] Probe failed: %s (%s) — %v", domain, ip, err)
		return
	}
	conn.Close()
	vh.RecordSuccess(domain, ip, rtt)
	util.TurnLog("[VKHosts] Probe OK: %s (%s) — RTT=%v", domain, ip, rtt)
}
