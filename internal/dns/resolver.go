...
func resolveWithOrderedServers(ctx context.Context, domain string) (string, error) {
	...
	for i := 0; i < len(dnsServers); i++ {
		...
		util.TurnLog("[DNS] Trying server %d (%v, %s) for %s", idx, server.Type, server.IP, domain)
		ip, err := resolveAny(ctx, domain, server)
		if err == nil {
			util.TurnLog("[DNS] Success with server %d: %s -> %s", idx, domain, ip)
		...
		util.TurnLog("[DNS] Server %d failed: %v", idx, err)
	}
...
func resolveUDPWithServer(ctx context.Context, domain string, serverIP string) (string, error) {
	...
	// Use DialContext with util.ProtectControl (same as old protectedResolver)
	addr := serverIP + ":53"
	dialer := &net.Dialer{
		Timeout: dnsTimeout,
	...
	client := &http.Client{
		...
		Transport: &http.Transport{
			DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
				return util.ProtectAndDial(ctx, network, serverIP+":443")
			},
...
func resolveDoTWithServer(ctx context.Context, domain string, serverIP string, serverName string) (string, error) {
	...
	// DoT uses port 853
	tcpConn, err := util.ProtectAndDial(ctx, "tcp", serverIP+":853")
	if err != nil {
...
func util.ProtectAndDial(ctx context.Context, network, addr string) (net.Conn, error) {
	dialer := &net.Dialer{
		...
		Control:   util.ProtectControl, // Protects socket before connect()
	}
...
func ClearCache() {
	...
	lastSuccessfulMu.Unlock()
	util.TurnLog("[DNS] Cache cleared")
}
...
func resolveWithOrderedServersAll(ctx context.Context, domain string) ([]string, error) {
	...
	for i := 0; i < maxServers; i++ {
		...
		idx := (i + startIndex) % len(dnsServers)
		server := dnsServers[idx]
		util.TurnLog("[DNS] Trying server %d (%v, %s) for %s (all, attempt %d/%d)", idx, server.Type, server.IP, domain, i+1, maxServers)
		...
		if err == nil && len(ips) > 0 {
			...
			util.TurnLog("[DNS] Server %d returned %d IPs (%d new) for %s: %v", idx, len(ips), newCount, domain, ips)
			// If we already have >=3 IPs, that's enough
			if len(allIPs) >= 3 {
				break
			}
		} else {
			util.TurnLog("[DNS] Server %d failed (all): %v", idx, err)
		}
	}
	...
	util.TurnLog("[DNS] Total %d unique IPs for %s: %v", len(allIPs), domain, allIPs)
	return allIPs, nil
}
...
func InitSystemDns(servers []string) {
	...
	util.TurnLog("[DNS] Initialized: %d system + %d predefined = %d total",
